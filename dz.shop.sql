...
UPDATE users
    SET created_at = NULL where id > 0;

# Далее следует домашнее задание

# Заполняем поля нынешним временем
INSERT INTO users (created_at, updated_at) VALUES (NOW(), NOW());

# Так как изначально была задана правильная кодеровка, а по заданию надо изменить с VARCHAR на DATETIME, то сначала придётся поменять на VARCHAR
 ALTER TABLE users 
 CHANGE COLUMN `created_at` `created_at` VARCHAR(100),
 CHANGE COLUMN `updated_at` `updated_at` VARCHAR(100);
 
 # Согласно условиям домашнего задания меняем на DATETIME
 ALTER TABLE users 
 ...
 # Сортировка записи таким образом, чтобы они выводились в порядке увеличения значения value. Однако нулевые запасы выводиться в конце, после всех 
 SELECT 
    value
FROM
